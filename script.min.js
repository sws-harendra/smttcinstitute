function openMenu(){document.getElementById("mobileMenu").classList.remove("hidden")}function closeMenu(){document.getElementById("mobileMenu").classList.add("hidden"),document.getElementById("servicesMenu").classList.add("hidden"),document.getElementById("serviceArrow").classList.remove("rotate-180")}function toggleServices(){let e=document.getElementById("servicesMenu"),n=document.getElementById("serviceArrow");e.classList.toggle("hidden"),n.classList.toggle("rotate-180")}const images=["assets/images/g1.webp","assets/images/g2.webp","assets/images/g3.webp",];const galleryImage=document.getElementById("galleryImage");if(galleryImage){let i=0;setInterval(()=>{i=(i+1)%images.length;galleryImage.src=images[i]},3000)}const yearElem=document.getElementById("year");if(yearElem){yearElem.textContent=new Date().getFullYear()}const GalleryPageimages=document.querySelectorAll(".gallery-img"),modal=document.getElementById("modal"),modalImg=document.getElementById("modalImg");function closeModal(){if(modal){modal.classList.add("hidden"),modal.classList.remove("flex")}}function openCourse(e){let n=document.getElementById("courseModal"),s=document.getElementById("courseTitle"),t=document.getElementById("courseContent");if(n&&s&&t){n.classList.remove("hidden"),n.classList.add("flex"),"ac"===e&&(s.innerText="AC Repairing Course",t.innerHTML=`
      <p class="mb-6">
...
This hands-on training approach makes sure students gain real repair experience instead of only theoretical knowledge. The course is designed to build confidence so students can handle customer laptops independently and with accuracy.
    ...
    `)}}function closeCourse(){let e=document.getElementById("courseModal");if(e){e.classList.add("hidden"),e.classList.remove("flex")}}window.openMenu=openMenu,window.closeMenu=closeMenu,window.toggleServices=toggleServices,window.openCourse=openCourse,window.closeCourse=closeCourse,window.closeModal=closeModal;GalleryPageimages.forEach(e=>{e.addEventListener("click",()=>{if(modal&&modalImg){modal.classList.remove("hidden"),modal.classList.add("flex"),modalImg.src=e.src}})}),document.querySelectorAll(".faq-btn").forEach(e=>{e.addEventListener("click",()=>{let n=e.nextElementSibling,s=e.querySelector("span");if(n&&s){n.classList.toggle("hidden"),s.textContent=n.classList.contains("hidden")?"+":"−"}})});
