function openMenu(){document.getElementById("mobileMenu").classList.remove("hidden")}function closeMenu(){document.getElementById("mobileMenu").classList.add("hidden"),document.getElementById("servicesMenu").classList.add("hidden"),document.getElementById("serviceArrow").classList.remove("rotate-180")}function toggleServices(){let e=document.getElementById("servicesMenu"),n=document.getElementById("serviceArrow");e.classList.toggle("hidden"),n.classList.toggle("rotate-180")}const slides=document.querySelectorAll(".carousel-slide"),dots=document.querySelectorAll(".dot");let current=0;function showSlide(e){slides.forEach((n,s)=>{n.classList.toggle("opacity-100",s===e),n.classList.toggle("opacity-0",s!==e),dots[s].classList.toggle("opacity-100",s===e),dots[s].classList.toggle("opacity-40",s!==e)})}setInterval(()=>{showSlide(current=(current+1)%slides.length)},4e3);const images=["assets/images/g1.webp","assets/images/g2.webp","assets/images/g3.webp",];const galleryImage=document.getElementById("galleryImage");if(galleryImage){let i=0;setInterval(()=>{i=(i+1)%images.length;galleryImage.src=images[i]},3000)}document.getElementById("year").textContent=new Date().getFullYear();const GalleryPageimages=document.querySelectorAll(".gallery-img"),modal=document.getElementById("modal"),modalImg=document.getElementById("modalImg");function closeModal(){modal.classList.add("hidden"),modal.classList.remove("flex")}function openCourse(e){let n=document.getElementById("courseModal"),s=document.getElementById("courseTitle"),t=document.getElementById("courseContent");n.classList.remove("hidden"),n.classList.add("flex"),"ac"===e&&(s.innerText="AC Repairing Course",t.innerHTML=`
      <p class="mb-6">
    ...
    `),"washing"===e&&(s.innerText="Washing Machine Repairing Course",t.innerHTML=`
      <p class="mb-6">
    ...
    `),"fridge"===e&&(s.innerText="Fridge Repairing Course",t.innerHTML=`
      <p class="mb-6">
...
This course gives strong importance to practical learning where students work on live repair cases under expert guidance. The main aim is to build confidence so students can independently attend service calls and handle customer repairs without hesitation.
    ...
    `),"mobile"===e&&(s.innerText="Mobile Repairing Course",t.innerHTML=`
      <p class="mb-6">
...
With smartphones now being a part of everyday life the need for skilled mobile repair technicians is growing fast. Mobile repairing continues to be one of the most reliable and fast growing technical career choices today.
    ...
    `),"laptop"===e&&(s.innerText="Laptop Repairing Course",t.innerHTML=`
      <p class="mb-6">
...
This hands-on training approach makes sure students gain real repair experience instead of only theoretical knowledge. The course is designed to build confidence so students can handle customer laptops independently and with accuracy.
    ...
    `)}function closeCourse(){let e=document.getElementById("courseModal");e.classList.add("hidden"),e.classList.remove("flex")}GalleryPageimages.forEach(e=>{e.addEventListener("click",()=>{modal.classList.remove("hidden"),modal.classList.add("flex"),modalImg.src=e.src})}),document.querySelectorAll(".faq-btn").forEach(e=>{e.addEventListener("click",()=>{let n=e.nextElementSibling,s=e.querySelector("span");n.classList.toggle("hidden"),s.textContent=n.classList.contains("hidden")?"+":"−"})});
